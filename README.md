...
如果你是拿到原始碼要自己編譯，請參考下方「開發者：如何編譯」章節；編譯完成後，把產生的檔案複製到任何一台同類型系統的電腦上即可，**不需要額外安裝 .NET**。

### 開啟方式

- **Windows**：直接**雙擊** `deployctl.exe`。
- **Linux**：在終端機執行：
  ```bash
...
不帶任何參數執行時，會直接進入**文字選單介面**——用上下方向鍵選擇項目、Enter 確認、依照畫面提示輸入名稱或路徑即可，完全不需要記任何指令。

第一次執行時會先請你選擇語言（English / 繁體中文），選擇後會記住設定，之後每次開啟都會直接使用同一個語言，也可以隨時在主選單的「語言設定」重新切換。
...
> 這一節示範的是 `deployctl <command> ...` 命令列操作方式，適合資訊人員寫自動化腳本使用。
> 如果你是透過雙擊執行檔開啟的**互動式選單**，操作方式是選單導覽，功能與下面完全對應，不需要輸入任何指令，可以跳過這節直接照選單畫面操作。
...
**Q: 語言設定存在哪裡？想要重設語言怎麼辦？**
語言偏好會存在 `DEPLOYCTL_DATA` 目錄下的 `cli-settings.json`（例如 Windows 預設是 `%APPDATA%\deployctl\cli-settings.json`）。可以在互動式選單的「語言設定」隨時切換，或直接刪除這個檔案，下次開啟時會重新詢問語言。

**Q: 雙擊執行檔沒有反應、或一閃就關掉？**
互動式選單需要在真正的終端機視窗中執行才能顯示（用來偵測方向鍵輸入）。如果是透過某些會重新導向輸出的方式啟動（例如某些捷徑設定、排程工具），程式會自動改顯示指令說明並結束，這是正常的防呆行為；直接雙擊執行檔或在命令提示字元/終端機裡執行就能看到選單。
